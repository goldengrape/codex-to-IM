...
function handleTelegramText(state, event) {
    ...
    if (!isAuthorized(nextState, event.userId, event.chatId)) {
        return withAudit(nextState, 'telegram_unauthorized', event, [sendMessage(event.chatId, '未授权的用户或会话，已拒绝此次请求。'), persist('unauthorized_text')]);
    }
    const binding = ensureBinding(nextState, event.chatId, event.at);
    nextState = binding.state;
    if (!binding.binding.workspace) {
        return withAudit(nextState, 'telegram_missing_workspace', event, [sendMessage(event.chatId, '请先使用 /cwd <workspace> 绑定工作目录。'), persist('missing_workspace')]);
    }
    if (binding.binding.mode === 'paused') {
        return withAudit(nextState, 'telegram_binding_paused', event, [sendMessage(event.chatId, '当前会话已暂停，恢复后才能继续执行。'), persist('binding_paused')]);
    }
    if (binding.binding.activeTurnId && binding.binding.threadId) {
        return withAudit(nextState, 'telegram_text_steer', event, [
    ...
    if (!binding.binding.threadId) {
        ...
        return withAudit(newState, 'telegram_text_start_thread', event, [
    ...
    return withAudit(nextState, 'telegram_text_start_turn', event, [
...
function handleTelegramCommand(state, event) {
    if (isDuplicateUpdate(state, event.chatId, event.updateId)) {
        return noOp(state);
    }
    let nextState = markProcessedUpdate(state, event.chatId, event.updateId);
    if (!isAuthorized(nextState, event.userId, event.chatId)) {
        return withAudit(nextState, 'telegram_unauthorized', event, [sendMessage(event.chatId, '未授权的用户或会话，已拒绝此次请求。'), persist('unauthorized_command')]);
    }
    const parsed = (0, commands_1.parseCommand)(event.commandText);
    if (isParseError(parsed)) {
        return withAudit(nextState, 'telegram_command_parse_error', { ...event, error: parsed.message }, [sendMessage(event.chatId, `命令解析失败：${parsed.message}`), persist('command_parse_error')]);
    }
    return applyCommand(nextState, event.chatId, event.at, parsed, event);
}
function handleApprovalClick(state, event) {
    if (isDuplicateUpdate(state, event.chatId, event.updateId)) {
        return noOp(state);
    }
    let nextState = markProcessedUpdate(state, event.chatId, event.updateId);
    if (!isAuthorized(nextState, event.userId, event.chatId)) {
        return withAudit(nextState, 'telegram_unauthorized_approval', event, [sendMessage(event.chatId, '未授权的用户或会话，无法处理审批。'), persist('unauthorized_approval')]);
    }
    return resolveApproval(nextState, event.approvalId, event.decision, event.chatId, event.at, event);
}
function handleThreadStarted(state, event) {
    ...
    const effects = [persist('thread_started')];
    if (pendingInput) {
        nextState = {
            ...nextState,
            pendingInputsByChatId: {
...
function handleTurnStarted(state, event) {
...
function handleAgentTextDelta(state, event) {
    ...
    const effects = [audit('agent_text_delta', { threadId: event.threadId, turnId: event.turnId })];
...
function handleDiffUpdated(state, event) {
    const view = state.liveViewsByThreadId[event.threadId];
    if (!view) {
...
function handleApprovalRequested(state, event) {
...
function handleTurnCompleted(state, event) {
    const located = findBindingByThreadId(state, event.threadId);
    const effects = [audit('turn_completed', event)];
    let nextState = state;
    if (located) {
        nextState = {
            ...nextState,
...
function applyCommand(state, chatId, at, command, auditPayload) {
    switch (command.kind) {
        case 'cwd':
...
function handleCwdCommand(state, chatId, at, workspace, auditPayload) {
...
function handleResetCommand(state, chatId, at, auditPayload) {
...
function updateJobEnabled(state, chatId, jobId, enabled, at, auditPayload) {
    const job = state.jobs[jobId];
    if (!job || job.chatId !== chatId) {
        return withAudit(state, 'job_not_found', auditPayload, [sendMessage(chatId, `任务不存在：${jobId}`)]);
    }
    const nextRunAt = enabled ? job.nextRunAt ?? (0, scheduler_1.computeNextRun)(job.schedule, at) : job.nextRunAt;
    const nextState = {
        ...state,
        jobs: {
            ...state.jobs,
...
function deleteJob(state, chatId, jobId, at, auditPayload) {
    const job = state.jobs[jobId];
    if (!job || job.chatId !== chatId) {
        return withAudit(state, 'job_not_found', auditPayload, [sendMessage(chatId, `任务不存在：${jobId}`)]);
    }
...
function addJobFromCommand(state, chatId, at, command, auditPayload) {
    ...
    const resolvedNextRunAt = schedule.kind === 'one_shot' ? schedule.runAt : (0, scheduler_1.computeNextRun)(schedule, at);
...
function handleJobDue(state, jobId, at, source) {
    const job = state.jobs[jobId];
...
function resolveApproval(state, approvalId, decision, chatId, at, auditPayload) {
...
function finalizeRun(state, runId, status, error, at) {
    ...
    if (job.schedule.kind === 'one_shot') {
        ...
        nextRunAt = (0, scheduler_1.computeNextRun)(job.schedule, at);
        enabled = true;
    }
    ...
    return {
        state: nextState,
        effects: [
            { type: 'ReleaseJobLock', jobId: job.jobId },
            { type: 'UpdateJobNextRun', jobId: job.jobId, nextRunAt },
        ],
    };
}
function recoverState(snapshot, at) {
    if (!snapshot) {
        return (0, types_1.createInitialState)({ lastRecoveryAt: at });
    }
    const runs = Object.fromEntries(Object.entries(snapshot.runs).map(([runId, run]) => {
    ...
    const activeRunByJobId = Object.fromEntries(Object.keys(snapshot.activeRunByJobId).map((existingJobId) => [existingJobId, null]));
    return {
        ...snapshot,
        runs,
        activeRunByJobId,
...
function renderStatus(state, chatId) {
    const binding = state.bindings[chatId];
    const jobs = Object.values(state.jobs).filter((job) => job.chatId === chatId);
    const pendingApprovals = Object.values(state.pendingApprovals).filter((approval) => approval.chatId === chatId && approval.status === 'pending');
    return [
...
function renderDiffSummary(state, chatId) {
    const binding = state.bindings[chatId];
    if (!binding?.threadId) {
...
function renderJobs(state, chatId) {
    const jobs = Object.values(state.jobs).filter((job) => job.chatId === chatId);
    if (jobs.length === 0) {
        return '当前没有任务。';
    }
...
function renderJob(state, jobId) {
    const job = state.jobs[jobId];
    if (!job) {
        return `任务不存在：${jobId}`;
    }
...
function renderLiveView(text, diff) {
    return diff ? `${text}\n\nDiff:\n${diff}` : text;
}
function renderRunCompletion(run) {
    return `任务 ${run.jobId} 已结束，状态：${run.status}${run.error ? `，错误：${run.error}` : ''}`;
}
function commandToSchedule(command) {
    switch (command.kind) {
        case 'job_add_once': {
            const runAt = (0, scheduler_1.toScheduledIsoFromLocalDateTime)(command.runAt, types_1.DEFAULT_TIMEZONE);
            return runAt ? { kind: 'one_shot', runAt, timezone: types_1.DEFAULT_TIMEZONE } : null;
        }
        case 'job_add_daily': {
            const hm = (0, scheduler_1.parseHourMinute)(command.hm);
            return hm ? { kind: 'daily', hour: hm.hour, minute: hm.minute, timezone: types_1.DEFAULT_TIMEZONE } : null;
        }
        case 'job_add_weekly': {
            const hm = (0, scheduler_1.parseHourMinute)(command.hm);
            return hm
                ? { kind: 'weekly', weekdays: command.weekdays, hour: hm.hour, minute: hm.minute, timezone: types_1.DEFAULT_TIMEZONE }
                : null;
        }
    }
}
function nextEntityId(prefix, existingIds) {
...
function buildJobName(prompt) {
    const trimmed = prompt.trim();
    return trimmed.length <= 32 ? trimmed : `${trimmed.slice(0, 29)}...`;
}
function ensureBinding(state, chatId, at, workspace) {
...
function findBindingByThreadId(state, threadId) {
...
function findActiveRunByThread(state, threadId) {
...
function findRunByThreadOrTurn(state, threadId, turnId) {
...
function clearActiveRun(state, jobId) {
    return {
        ...state,
        activeRunByJobId: {
            ...state.activeRunByJobId,
...
function isAuthorized(state, userId, chatId) {
    return ((state.acl.users.includes(userId) || state.acl.admins.includes(userId)) &&
        state.acl.chats.includes(chatId));
}
function isWorkspaceAllowed(state, workspace) {
    return state.acl.workspaces.includes(workspace);
}
function isDuplicateUpdate(state, chatId, updateId) {
    return updateId <= (state.processedUpdateIds[chatId] ?? -1);
}
function markProcessedUpdate(state, chatId, updateId) {
    return {
        ...state,
        processedUpdateIds: {
...
function noOp(state, effects = []) {
    return { newState: state, emittedEvents: [], effects };
}
function withAudit(state, topic, payload, effects) {
    return {
        newState: state,
        emittedEvents: [],
